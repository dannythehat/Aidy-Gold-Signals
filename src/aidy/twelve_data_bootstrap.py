...
from .twelve_data_market import AIDY_SYMBOL, RAW_M1_SOURCE, TwelveDataOhlcGateway
...
def plan_bootstrap_windows(
    as_of: datetime, *, latest_m1_open_utc: datetime
) -> tuple[BootstrapWindow, ...]:
    required = sorted(
        bootstrap_required_m1_open_times(
            _utc(as_of), latest_m1_open_utc=_utc(latest_m1_open_utc)
        )
    )
    if not required:
        return ()
    ...
    for opened in required[1:]:
        if (
            opened - bucket[-1] == timedelta(minutes=1)
            and len(bucket) < MAX_BOOTSTRAP_WINDOW_MINUTES
        ):
    ...
    async def ingest_window(
        ...
        if missing:
            ...
            return {
                ...
                "window_index": window.index,
                "required_m1_minutes": len(window.required_opens),
                "missing_m1_minutes": len(missing),
                "persisted_new_m1_minutes": 0,
                "state": "failed",
        ...
        try:
            ...
            await self._store.finish_bootstrap_window(
                ...
                window_index=window.index,
                state="failed",
                persisted_m1_minutes=persisted,
                response_digest=fetch.response_digest,
        ...
        return {
            ...
            "window_index": window.index,
            "required_m1_minutes": len(window.required_opens),
            "missing_m1_minutes": 0,
            "persisted_new_m1_minutes": persisted,
            "state": "succeeded",
            "decision_snapshot_created": False,
            "decision_ready": False,
        }
