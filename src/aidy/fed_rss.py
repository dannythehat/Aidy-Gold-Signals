"""Official Federal Reserve RSS capture for AIDY Gold Signals.
...
class AidyFedRssRecorderManager:
    """Independent official-event loop; failures never stop market capture."""

    def __init__(self, capture_once, *, poll_seconds: float = 120.0, sleep=asyncio.sleep) -> None:
    ...
    async def _run(self) -> None:
        while True:
            try:
                await self._capture_once(now=datetime.now(UTC))
            except asyncio.CancelledError:
                raise
            except Exception:
